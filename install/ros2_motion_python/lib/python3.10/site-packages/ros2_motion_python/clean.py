...
class RoboCleaner(Node):
    ...
    def __init__(self):
        # robocleaner below is the name of the node
        super().__init__('robocleaner')
        # /turtle1/cmd_vel is the topic name and we are publishing it
        self.velocity_publisher = self.create_publisher(Twist, '/turtle1/cmd_vel', 10)
        # Create a subscription to /turtle1/pose. The returned object is stored in self.pose_subscriber to
        # keep the subscription active. Every time a Pose message arrives, self.pose_callback is called with 
        # that message.
        self.pose_subscriber = self.create_subscription(Pose, "/turtle1/pose", self.pose_callback, 10)

        #How often the timer callback will be called
        timer_period = 0.01
        time.sleep(1.0)
        # creating an object of RobotPose which controls the direction of the robot
        self.pose = RobotPose()
        # The line spin hold the purpose of processing incoming messages or events exactly once for node
        rclpy.spin_once(self)

        # What do each of the lines below do? 
        self.client = self.create_client(Empty, 'reset')
        self.request = Empty.Request()

        # Sending your request to the service 
        self.response = self.client.call_async(self.request)
        # Keep checking until we receive a response
    ...
    def move (self, linear_speed, distance, is_forward):
        ...
        # making sure we are within the distance 
    ...
    def go_to_goal(self, goal: RobotPose, distance_error_tolerance=0.5):
        ...
        while (True):
            ...
            p_gain_linear = 1.0
            # This is the distance equation we explored earlier  
